...
const GanttChart = ({ institutions }) => {
  ...
  const getGanttPosition = (dateStr) => {
    ...
    try {
      ...
      return Math.max(0, Math.min(35, diffWeeks));
    } catch {
      return null;
    }
  };
          ...
          {months.map((month, idx) => (
            <div key={month} className="gantt-month">
              ...
              <div className="gantt-weeks">
                {[1, 2, 3, 4].map(w => (
                  <div key={w} className="gantt-week">{w}</div>
                ))}
              </div>
            </div>
          ))}
        ...
        {institutions.map((inst, idx) => (
          <div key={idx} className="gantt-row-group">
            <div className="gantt-institution-row">
              ...
              <div className="gantt-timeline">
                {inst.activities && inst.activities.length > 0 && (() => {
                  ...
                  const positions = validActivities.map(a => ({
            ...
            {inst.activities?.map((activity, aidx) => {
              ...
              let barClass = 'gantt-bar-activity-progress';
              if (activity.status === 'Concluído') {
                barClass = 'gantt-bar-activity-completed';
              } else if (activity.status === 'Projetado' || activity.status === 'Planejado') {
                barClass = 'gantt-bar-activity-planned';
              }
    ...
    <div className="institution-card">
      <div className="card-header">
        <div className="card-title-row">
          ...
          <span className={`status-badge ${statusColors[institution.status] || 'status-gray'}`}>
            {institution.status}
          </span>
        </div>
            ...
            {institution.activities.map((activity, idx) => (
              <div key={idx} className="activity-item">
                ...
                {activity.responsible && (
                  <div className="activity-resp">Responsável: {activity.responsible}</div>
                )}
                {activity.startDate && activity.endDate && (
                  <div className="activity-dates-display">
                    <Calendar size={12} />
                    {new Date(activity.startDate).toLocaleDateString('pt-BR')} até {new Date(activity.endDate).toLocaleDateString('pt-BR')}
                  </div>
                )}
              </div>
            ))}
                  ...
                  <div key={idx} className="activity-item-form">
                    <div className="activity-info">
                      ...
                      <div className="activity-meta">
                        ...
                        {activity.responsible && (
                          <span className="resp-tag">Resp: {activity.responsible}</span>
                        )}
                        {activity.startDate && activity.endDate && (
...
export default function MonitoringSystem() {
  ...
  const initialInstitutions = [
    {
      ...
      activities: [
        ...
        { name: 'Apresentação da arquitetura do sistema', responsible: 'ACC', startDate: '2026-05-16', endDate: '2026-06-15', status: 'Projetado' }
      ]
    },
    {
      id: 2,
    ...
    },
    {
      ...
      observations: '',
      activities: [
        { name: 'Projeto entregue', responsible: 'ACC', startDate: '2026-04-01', endDate: '2026-04-30', status: 'Concluído' }
      ]
    },
    {
      id: 5,
    ...
    },
    {
      ...
      activities: [
        { name: 'Reunião de definição', responsible: 'ACC', startDate: '2026-05-08', endDate: '2026-05-08', status: 'Projetado' }
      ]
    },
    {
      id: 8,
  ...
  const [institutions, setInstitutions] = useState(() => {
    if (typeof window === 'undefined') return initialInstitutions;
    try {
    ...
    if (editingInstitution) {
      setInstitutions(institutions.map(inst => 
        inst.id === editingInstitution.id 
          ? { ...institutionData, id: editingInstitution.id, activities: institutionData.activities || [] }
          : inst
      ));
    } else {
      setInstitutions([...institutions, { 
        ...institutionData, 
        id: Date.now(),
        activities: institutionData.activities || []
      }]);
    }
    setShowForm(false);
    ...
    <div className="app">
      ...
      <style jsx global>{`
        * {
        ...
        .resp-tag.activity-status-green { background: #168821; color: white; }
        .resp-tag.activity-status-blue { background: #1351B4; color: white; }
        .resp-tag.activity-status-orange { background: #FF8C00; color: white; }
        ...
        .legend-green { background: #168821; }
        .legend-blue { background: #1351B4; }
        .legend-orange { background: #FF8C00; }
